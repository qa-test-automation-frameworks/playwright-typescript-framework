...
| Profiles            | `tests/api/profile.api.spec.ts`                                                         | `tests/e2e/user-journey.spec.ts`                                | Not covered             | Not covered                      | Profile article lists and social relationships are checked in API/UI.                             |
| Social graph        | `tests/api/profile.api.spec.ts`, `tests/api/controlled-target-regressions.api.spec.ts`  | `tests/e2e/user-journey.spec.ts`                                | Not covered             | Not covered                      | Follow/unfollow and personal feed behavior are critical-path checks.                              |
...
| Cross-browser smoke | Not applicable                                                                          | `firefox-smoke`, `webkit-smoke`, `mobile-chrome-smoke` projects | Not covered             | Not covered                      | Smoke-tagged browser flows validate portability without multiplying the whole suite.              |
